defmodule Aggregator do
  ...
  def handle_cast(:get_tweet, state) do
    state =
      (state.finished_tweets != [] && send_finished_tweet(state)) ||
        %{state | batcher_request: true}
    ...
    if tweet? do
      ...
      tweet = %{
        ...
        | text: (tweet.redact_p && tweet.text) || existing_tweet.text,
          sentimental_score:
            (tweet.sentiment_p && tweet.sentimental_score) || existing_tweet.sentimental_score,
          engagement_ratio:
            (tweet.engagement_p && tweet.engagement_ratio) || existing_tweet.engagement_ratio,
